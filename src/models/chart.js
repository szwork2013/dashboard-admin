...
* @Last modified time: 2017-04-19T16:49:34+08:00
* @License: MIT
...
export default {
  ...
  reducers: {
    ...
    'layout/remove'(state, { payload: key }) {
      return {
        ...state,
        layout: state.layout.filter(e => e.i !== key),
        charts: state.charts.filter(e => e.key !== key),
        selectedId: null,
      };
    },
    // chart
    'achart/select'(state, { payload: { key, type, option, data } }) {
      const { charts } = state;
      const keys = charts.map(e => e.key);

      const index = keys.indexOf(key);
      const chart = index === -1 ? { key, type }
        : { data, ...charts[index], type, option }; // app data > initial data
    ...
    'achart/change'(state, { payload: { key, type, data, option } }) {
      return {
        ...state,
        charts: state.charts.map(e => (e.key === key ? { ...e, type, data, option } : e)),
      };
    },
